    ...
    for (byte i = 0; i < 32; i++) {
        ...
        if (intFlags[i]) {
            ...
            if(Serial){
                ...
                Serial.print(i);
                Serial.print(" is set. ");
                Serial.print(InterruptTable[i].name);
                Serial.print(" status: ");
                for(byte j = 0; j < InterruptTable[i].nRegs ; j++){
                    Serial.print(MP2790::readAdd(InterruptTable[i].regs[j]));
                }
                Serial.println();
            }
        }
...
void MP2790::clearInt(uint8_t interruptNum) {

    if (interruptNum < 16){
        RegDes reg = {INT0_CLR, interruptNum, 1, RWAccess::READ_WRITE};
        MP2790::writeAdd(reg, 1);
    }
    else{
        RegDes reg = {INT1_CLR, interruptNum-16, 1,RWAccess::READ_WRITE};
        MP2790::writeAdd(reg, 1);
    }
...
bool MP2790::getFault(bool *faultFlags) {
    bool faultStatus = false;
    uint32_t tempFlags = 0;
    uint16_t ft1 =  this->readAdd(FT_STS1);
    uint16_t ft2 =  this->readAdd(FT_STS2);
    tempFlags = (uint32_t)ft1 | (uint32_t)ft2 << 16;
    ...
    for (byte i = 0; i < 32; i++) {
        faultFlags[i] = (tempFlags >> i) & 0x01;
        if (faultFlags[i]) {
            faultStatus = true;
            if(Serial){
                Serial.print("Fault Flag ");
                Serial.print(i);
                Serial.print(" is triggered. ");
...
void MP2790::clearFault(uint8_t faultNum) {

    Serial.print("fault  num: ");
    Serial.println(faultNum);


    // Check if fault conditions are active
    // for ( int j = 0; j < FaultTable[faultNum].nRegs ;j++){
    //     // if (MP2790::readAdd(FaultTable[faultNum].regs[j]) == 1){
    //     //     Serial.println("True");
    //     // }    
    // }

    // Serial.print("nclear :");
    //     Serial.println(FaultTable[faultNum].nClear);

    // Clears realted registers
    if (FaultTable[faultNum].nClear != 0){
        for ( int j = 0; j < FaultTable[faultNum].nClear ;j++){
            MP2790::writeAdd(FaultTable[faultNum].clear[j], 1);    
        }
    }

    // if (faultNum == 10){
    //     MP2790::writeAdd(MP2790_Reg::ACTIVE_CTRL, 0);    
    //     MP2790::writeAdd(MP2790_Reg::DRIVER_FAULT_CLR, 1);
    //     delay(1000);
        // MP2790::writeAdd(MP2790_Reg::ACTIVE_CTRL, 3);
        // Serial.println(MP2790::readAdd(MP2790_Reg::DRIVER_FAULT_STS));
    // }
    // if (faultNum < 16){
    //     RegDes reg = {FT_CLR, faultNum, 1, RWAccess::READ_WRITE};
    //     MP2790::writeAdd(reg, 1);
    // }
    // else if (faultNum == 16){
    //     RegDes reg = {FT_REC, 14, 1, RWAccess::READ_WRITE};
    //     MP2790::writeAdd(reg, 1);
    // }
    // else if (faultNum == 17 || faultNum == 18){
    //     RegDes reg = {FT_REC, 15, 1, RWAccess::READ_WRITE};
    //     MP2790::writeAdd(reg, 1);
    // }    
    // else if (faultNum == 19){
    //     RegDes reg = {FT_REC, 13, 1, RWAccess::READ_WRITE};
    //     MP2790::writeAdd(reg, 1);
    // }    
    // else if (faultNum == 29){
    // }
...
// void returnInterrupt(uint8_t interruptNum) {
//     if (interruptNum > 0 && interruptNum < 32) {
//         switch (interruptNum)
//         {
//         case 0:
 ...
 */
void MP2790::getNTCreadings(int32_t* ntcValues) {
    ...
    for (int i = 0; i < 4; i++) {
        double rawValue = this->readAdd(ntc_regs[i]);       
        // Convert the raw 10-bit value to a percentage of the reference voltage.
        ntcValues[i] = MP2790::tempConversion(rawValue / 1023.0f);    
    }
    return;
}
...
uint16_t MP2790::getDieTemperature() {
    double rawValue = this->readAdd(MP2790_Reg::DIE_TEMP_V);
    // Apply the conversion formula from the datasheet.
    return (uint16_t)((rawValue * 0.474f - 269.12f) * 100.0f);
...
HRDataPacket MP2790::getADCVals(){

    getHRCellReadings(hrData.cellHRVoltage, hrData.cellHRCurrent);
    getHRNTCReadings(hrData.ntcHRValues);
    getHRGPIOReadings(hrData.gpioHRValues);
    getHRADCReadings(hrData.adcHRValues);
    hrData.packPReadings = getPackPReadings();
    hrData.vTopReadings = getVTopReadings();
    hrData.iTopReadings = getITopReadings();
    hrData.dieTemp = getHRDieTemperature();
...
/**
 * @brief Handles the HR ADC scan sequence
 * @param trigger 0: check || 1: trigger || 2: retrieve
 */
void MP2790::triggerHRScan(uint8_t trigger){

    static uint8_t scanState = 0;
    static uint32_t valueTimeout = 0;

    switch (scanState){
        //Checks for trigger to start sequence
        case 0:{
            if(trigger == 1){
                //Deactivates scan go if latched
                if(MP2790::readAdd(ADC_SCAN_GO) == 1){
                    MP2790::writeAdd(MP2790_Reg::ADC_SCAN_GO,0);
                }
                if(Serial){
                    Serial.println("ADC HR scan go...");
                }
                scanState = 1;
                valueTimeout = millis();
            }
    ...
    //Expires the data
    if((scanState != 0) && (millis() - valueTimeout > 1000)){
        hrData = {0};
        scanState = 0; 
        if(Serial){
            Serial.println("ADC HR cleared");
        }
        //Unlatches go command
        MP2790::writeAdd(MP2790_Reg::ADC_SCAN_GO,0);  
    }
    return;
}
...
HRDataPacket MP2790::getHRADCVals(){
...
int16_t MP2790::getITopReadings() {
    double rawValue = (double)this->readAdd(MP2790_Reg::VTOP_I); // Cast to signed
    // Convert from raw ADC value to mA using the sense resistor value.
    // // return rawValue;
    // Serial.print("Raw iTop: ");
    // Serial.println(readAdd(MP2790_Reg::VTOP_I), BIN);
    // Serial.println(readAdd(MP2790_Reg::VTOP_I), DEC);
    // Serial.print("Raw iTop double: ");
    // Serial.println(rawValue, BIN);
    // Serial.println(rawValue, DEC);
    double returnVal = rawValue * 100.0f * 1000.0f / 32768.0f / MP2790_R_SENSE;
    // Serial.print("Raw return: ");
    // Serial.println(returnVal, BIN);
    // Serial.println(returnVal, DEC);
    // Serial.print("typecast return: ");
    // Serial.println((int16_t)returnVal, BIN);
    // Serial.println((int16_t)returnVal, DEC);
...
void MP2790::getHRCellReadings(uint16_t* cellHRVoltage, int16_t* cellHRCurrent) {
    ...
    for (int i = 0; i < this->_nCells; i++) {
        ...
        double rawCurrent = (double)this->readAdd(i_regs[i]);
        cellHRCurrent[i] = (int16_t)(rawCurrent * 1000.0f * 100.0f  / 32768.0f / MP2790_R_SENSE);
    }
}
...
void MP2790::getHRNTCReadings(uint16_t* ntcHRValue) {
    ...
    for (int i = 0; i < 4; i++) {
        double rawValue = this->readAdd(ntc_hr_regs[i]);
        // Convert the raw 15-bit value to a percentage of the reference voltage.
        ntcHRValue[i] = MP2790::tempConversion(rawValue / 32768.0f);        
    }
}

int32_t MP2790::tempConversion(double rawValue) {
    //Caracterizacion NTC
    static const double NTC_SH_k[3] = {0.001178f, 0.0002322f, -0.00000001494f};

    double Rntc = 10000.0f * rawValue / (1.0f - rawValue);
    double Tntc = powl(
        NTC_SH_k[0] + NTC_SH_k[1]*logl(Rntc) +NTC_SH_k[2]*powl(logl(Rntc), 3),
        -1)
        - 273.15f;

    return (int32_t)(Tntc*100);
}
...
uint16_t MP2790::getHRDieTemperature() {
    double rawValue = this->readAdd(MP2790_Reg::DIE_T_VOLTAGE);
    // Serial.print("Raw_temp_HR: ");
    // Serial.println(rawValue);
    return (uint16_t)((rawValue * 0.014181f - 269.12f) * 100.0f);
    // return (uint16_t)(rawValue);
}
