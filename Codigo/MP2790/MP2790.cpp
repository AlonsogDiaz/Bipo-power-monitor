...
void MP2790::getNTCreadings(int32_t* ntcValues) {
    ...
    //Caracterizacion NTC
    float NTC_SH_k[3] = {0.001183f, 0.0005336f, -0.0000002233f};

    for (int i = 0; i < 4; i++) {
        float rawValue = this->readAdd(ntc_regs[i]);
        // Convert the raw 10-bit value to a percentage of the reference voltage.
        rawValue = (rawValue / 1023.0f);        
        double Rntc = 10000.0f *rawValue / (1.0f - rawValue);
        double Tntc = powl(
            NTC_SH_k[0] + NTC_SH_k[1]*log10l(Rntc) +NTC_SH_k[2]*powl(log10l(Rntc), 3),
            -1) - 273.15f;
        ntcValues[i] = (int32_t)(Tntc*1000);
    }
}
...
uint16_t MP2790::getDieTemperature() {
    ...
    float degreeValue = rawValue * 0.474f - 269.12f;
    // Apply the conversion formula from the datasheet.
    return (uint16_t)(degreeValue*1000);
 ...
 * @param trigger 0 : check || 1 : trigger || 2 : get
 */
...
int16_t MP2790::getITopReadings() {
    ...
    // return rawValue;
    return (int16_t)(rawValue * 100.0f * 1000.0f / 32768.0f / (MP2790_R_SENSE) );
}
...
void MP2790::getHRCellReadings(uint16_t* cellHRVoltage, int16_t* cellHRCurrent) {
    ...
    for (int i = 0; i < this->_nCells; i++) {
        ...
        cellHRVoltage[i] = (uint16_t)(rawVoltage * 50000.0f / 32768.0f);
...
void MP2790::getHRNTCReadings(uint16_t* ntcHRValue) {
    ...
    for (int i = 0; i < 4; i++) {
        float rawValue = this->readAdd(ntc_hr_regs[i]);
        ntcHRValue[i] = (uint16_t)(rawValue * 33000.0f / 32768.0f); // Assuming 3.3V reference
    }
}
