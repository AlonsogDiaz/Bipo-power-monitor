function varargout = aa_splot(varargin)
%AA_SPLOT replaces plot command and automatically changes color
%
%  This function is a replacement for the plot command.  
%  It automatically changes the color with subsequent
%  uses of aa_splot.  
%  
%  Usage is exactly the same as for plot.  

% (new)
% set(groot, 'defaultAxesColorOrder', colororder);
% 
% np = get(gca,'nextplot');
% oldplots = get(gca,'children');
% 
% cord = get(gca,'colororder');
% 
% if ~isempty(oldplots)
% lastcolor = get(oldplots(1),'color');
% if lastcolor == cord(1,:)
%   set(gca,'colororder',cord(mod([0:6]+1,7)+1,:));
% end
% end
% 
% hold on;
% 
% h = plot(varargin{:});
% 
% for IND = 1:nargout
%    varargout(IND) = {h};
% end
% 
% %if nargout > 0, varargout{:} = h; end;
% 
% set(gca,'colororder',cord(mod([0:6]+1,7)+1,:));
% set(gca,'nextplot',np);
% set(gca,'box','on');

hold on;
    % Get the current ColorOrder and determine which color is next
    ax = gca;
    color_order = get(ax, 'ColorOrder');
    index = get(ax, 'ColorOrderIndex');
    current_color = color_order(index, :);
    
    % Execute the plot
    h = plot(varargin{:});
    
    % Match the Y-Axis color to the line color
    set(ax, 'YColor', current_color);
    
    % Return handle if requested
    if nargout > 0
        varargout{1} = h;
    end
end