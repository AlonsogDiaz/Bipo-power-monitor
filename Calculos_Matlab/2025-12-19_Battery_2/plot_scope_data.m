function plot_scope_data(fileName, labelInfo, axisLimits, fileOutputName)
...
%       plot_scope_data('scope_38.csv', labels, limits);
    ...
    y1Name = labelInfo{2};
    % y2Name = labelInfo{3};
    plotTitle = labelInfo{3};
    plotSubTitle = labelInfo{4};
    ...
    % --- Primary Y-Axis (Left) ---
    % yyaxis left
    plot(time, signal1, 'LineStyle', '-', 'LineWidth', 1);
    ylabel(y1Name);
    grid on;
